...
const GitHub = {
  ...
  async getFile(path) {
    ...
    // 파일이 1MB 초과면 content 필드가 잘려서 오므로
    // raw.githubusercontent.com 엔드포인트로 직접 받는다
    const rawUrl = `https://raw.githubusercontent.com/${this.cfg.owner}/${this.cfg.repo}/${this.cfg.branch}/${path}`;
    // raw.githubusercontent.com은 Authorization 헤더가 있으면 CORS 차단됨
    // Public 저장소는 토큰 없이 읽을 수 있음
    const rawRes = await fetch(rawUrl);
    if (!rawRes.ok) throw new Error(`raw download ${path} 실패: ${rawRes.status}`);
