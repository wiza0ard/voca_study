...
const GitHub = {
  ...
  loadCfg() {
    ...
    // 구버전 cfg(owner/repo 없이 token만 있거나 키 구조가 다른 경우) 호환 처리
    if (!parsed.owner || !parsed.repo) {
      // token은 살리고 나머지는 초기화 → init()에서 setup 모달로 유도됨
      this.cfg = parsed.token ? { token: parsed.token } : null;
    } else {
      this.cfg = parsed;
    }
    return this.cfg;
  ...
  // 토큰으로 GitHub /user API를 호출해 username을 자동 취득
  async resolveOwner(token) {
    const res = await fetch('https://api.github.com/user', {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json' },
    });
    if (!res.ok) throw new Error(`토큰 인증 실패 (${res.status}). 토큰을 확인하세요.`);
  ...
  async getFile(path) {
    // 1단계: Contents API로 sha 취득
    const metaRes = await fetch(`${this.apiBase()}/${path}?ref=${this.cfg.branch}`, {
      headers: {
        Authorization: `Bearer ${this.cfg.token}`,
        Accept: 'application/vnd.github+json',
      },
    });
    if (metaRes.status === 404) return { data: null, sha: null, notFound: true };
    if (!metaRes.ok) throw new Error(`GET ${path} 실패: ${metaRes.status} ${await metaRes.text()}`);
    const meta = await metaRes.json();
    this.shaCache[path] = meta.sha;

    // 2단계: content 필드가 잘리지 않았으면 바로 파싱 (소용량 파일)
    if (meta.content && meta.encoding === 'base64') {
      try {
        return { data: JSON.parse(this.decode(meta.content)), sha: meta.sha };
      } catch (e) {
        // 잘린 경우 아래 blob API로 폴백
      }
    }

    // 3단계: Git Blobs API 사용 (파일 크기 제한 없음)
    const blobRes = await fetch(
      `https://api.github.com/repos/${this.cfg.owner}/${this.cfg.repo}/git/blobs/${meta.sha}`,
      {
        headers: {
          Authorization: `Bearer ${this.cfg.token}`,
          Accept: 'application/vnd.github+json',
  ...
  async testConnection() {
    ...
    if (res.status === 404) throw new Error(`저장소 '${this.cfg.repo}'를 찾을 수 없습니다. GitHub에 저장소가 있는지 확인하세요.`);
    if (!res.ok) throw new Error(`저장소 접근 실패 (${res.status}). 토큰 권한(Contents Read & Write)을 확인하세요.`);
    return true;
  },
};
...
function setupSetupModal() {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    if (!token) {
      errEl.textContent = 'Personal Access Token을 입력하세요.';
      errEl.classList.remove('hidden');
      return;
    }

    saveBtn.disabled = true;
    saveBtn.textContent = '사용자 정보 확인중...';
    errEl.classList.add('hidden');

    try {
      // 토큰으로 GitHub 사용자명 자동 취득
      const owner = await GitHub.resolveOwner(token);
      const repo = 'voca_study';
      const branch = 'main';

      GitHub.saveCfg({ owner, repo, branch, token });

      saveBtn.textContent = '저장소 연결 확인중...';
...
async function init() {
  ...
  if (cfg && cfg.token) {
    if (!cfg.owner) {
      // 구버전 cfg: token은 있으나 owner가 없음 → 자동 복구 시도
      try {
        setSyncStatus('syncing', '계정 정보 복구중...');
        const owner = await GitHub.resolveOwner(cfg.token);
        GitHub.saveCfg({ owner, repo: 'voca_study', branch: 'main', token: cfg.token });
        startApp();
      } catch (e) {
        // 토큰 자체가 만료됐거나 잘못된 경우 → setup 모달로
        document.getElementById('setup-modal').classList.remove('hidden');
        prefillSetupForm();
      }
    } else {
